...
{
    /// <summary>
    /// Menu de contexto de shell &quot;autônomo&quot;
    /// 
    /// Não está realmente depurado, mas está funcionando principalmente.
    /// Crie uma instância e chame Mostrar Menu de Contexto com uma lista de Informações de Arquivo para os arquivos.
    /// A limitação é que ele só lida com arquivos no mesmo diretório, mas pode ser corrigido
    /// alterando a maneira como os arquivos são traduzidos em PIDLs.
    /// 
    /// Baseado no navegador de arquivos em C # do projeto de código
    /// http://www.codeproject.com/useritems/FileBrowser.asp
    /// 
    /// Aula de hooking tirada da coluna Cutting Edge da MSDN Magazine
    ...
    {
        #region Constructor
        /// <summary>Constructo padrãor</summary>
       ...
       /// Este método recebe mensagens da janela. Isso fará com que as opções &quot;Abrir com&quot; e &quot;Enviar para&quot; funcionem
        ...
        protected override void WndProc(ref Message m)
        {
                ...
                ((int)ShellHelper.HiWord(m.WParam) & (int)MFT.POPUP) == 0)
            {
                ...
                if (ShellHelper.LoWord(m.WParam) == (int)CMD_CUSTOM.ExpandCollapse)
                    info = "Expande ou reduz o item selecionado atualmente";
                else
                {
                    info = "";
                }
        ...
        #region ReleaseAll()
        /// <summary>
        ///Libere todas as interfaces alocadas, PIDLs 
        /// </summary>
        ...
        /// <summary>
        ///Obtém a pasta da área de trabalho
        /// </summary>
        /// <returns>Pasta IShell para desktopfolder</returns>
        private IShellFolder GetDesktopFolder()
        {
            ...
            if (null == _oDesktopFolder)
            {
                ...
                if (S_OK != nResult)
                {
                    throw new ShellContextMenuException("Falha ao obter a pasta do shell da área de trabalho");
                }
        ...
        private IShellFolder GetParentFolder(string folderName)
        {
            if (null == _oParentFolder)
            {
                ...
                // Obtenha o PIDL para a pasta em que o arquivo está
                ...
                // Obtenha a pasta IShell para a pasta
        ...
        // Recupera a interface da pasta IShell para a pasta da área de trabalho, que é a raiz do namespace do Shell.
        [DllImport("shell32.dll")]
        private static extern Int32 SHGetDesktopFolder(out IntPtr ppshf);

        // Obtém uma estrutura STRRET retornada por IShell Folder :: Get Display Name Of, converte-a em uma string e coloca o resultado em um buffer.
        ...
        // A função Track Popup Menu Ex exibe um menu de atalho no local especificado e rastreia a seleção de itens no menu de atalho.
        // O menu de atalho pode aparecer em qualquer lugar da tela.
        [DllImport("user32.dll", ExactSpelling = true, CharSet = CharSet.Auto)]
        private static extern uint TrackPopupMenuEx(IntPtr hmenu, TPM flags, int x, int y, IntPtr hwnd, IntPtr lptpm);

        //A função Criar Menu Popup cria um menu suspenso, submenu ou menu de atalho. O menu está inicialmente vazio.
        //Você pode inserir ou acrescentar itens de menu usando a função Inserir item de menu. Você também pode usar
        //a função Inserir Menu para inserir itens de menu e a função Anexar Menu para anexar itens de menu.
        [DllImport("user32", SetLastError = true, CharSet = CharSet.Auto)]
        private static extern IntPtr CreatePopupMenu();

        // A função Destroy Menu destrói o menu especificado e libera qualquer memória que o menu ocupe.
        [DllImport("user32", SetLastError = true, CharSet = CharSet.Auto)]
        ...
        // Contém informações estendidas sobre um comando de menu de atalho
        ...
        // Um ​​identificador de memória global generalizado usado para operações de transferência de dados pelo
        // IAdvise Sink, IData Object e interfaces IOle Cache
        [StructLayout(LayoutKind.Sequential)]
        ...
        // Define as coordenadas xey de um ponto
        [StructLayout(LayoutKind.Sequential, CharSet = CharSet.Auto)]
        ...
        // Define os valores usados ​​com IShell Folder :: Get Display Name Of e IShell Folder :: Set Name Of
        // métodos para especificar o tipo de arquivo ou nomes de pasta usados ​​por esses métodos
        [Flags]
        private enum SHGNO
        {
            NORMAL = 0x0000,
        ...
        // Os atributos que o chamador está solicitando, ao chamar IShell Folder :: Get Attributes Of
        [Flags]
        ...
        // Determina o tipo de itens incluídos em uma enumeração.
        //Esses valores são usados ​​com o método IShell Folder :: Enum Objects
        [Flags]
        ...
        // Especifica como o menu de atalho pode ser alterado ao chamar IContext Menu :: Query Context Menu
        [Flags]
        ...
        // Sinalizadores que especificam as informações a serem retornadas ao chamar IContext Menu :: Get Command String
        [Flags]
        ...
        //Especifica como o Track Popup Menu Ex posiciona o menu de atalho horizontalmente
        [Flags]
        ...
        // O cmd para um item de menu adicionado personalizado
        private enum CMD_CUSTOM
        {
            ExpandCollapse = (int)CMD_LAST + 1
        }

        // Sinalizadores usados ​​com a estrutura CMINVOKECOMMANDINFOEX
        [Flags]
        ...
        // Especifica como a janela deve ser mostrada
        [Flags]
        private enum SW
        {
            HIDE = 0,
        ...
        // Sinalizadores de mensagem da janela
        [Flags]
        ...
        // Especifica o conteúdo do novo item de menu
        [Flags]
        ...
        // Especifica o estado do novo item do menu
        [Flags]
        ...
        // Indica o tipo de meio de armazenamento sendo usado em uma transferência de dados
        [Flags]
        private enum TYMED
        {
            ENHMF = 0x40,
        ...
        {
            //Traduz o nome de exibição de um objeto de arquivo ou pasta em uma lista de identificadores de item.
            ...
            // Recupera um objeto IShell Folder para uma subpasta.
            // Valor de retorno: código de erro, se houver
            [PreserveSig]
            Int32 BindToObject(
                IntPtr pidl,
           ...
           // Determina a ordem relativa de dois objetos de arquivo ou pastas, dados seus
            ...
            // valor de retorno negativo indica que o primeiro item deve preceder
            // o segundo (pidl1 &lt;pidl2).

            // Positivo Um valor de retorno positivo indica que o primeiro item deve
            // siga o segundo (pidl1&gt; pidl2). Zero Um valor de retorno de zero
            // indica que os dois itens são iguais (pidl1 = pidl2).
            [PreserveSig]
            Int32 CompareIDs(
                IntPtr lParam,
                IntPtr pidl1,
            ...
            // Solicita um objeto que pode ser usado para obter informações ou interagir
            // com um objeto de pasta.
            // Valor de retorno: código de erro, se houver
            [PreserveSig]
            Int32 CreateViewObject(
                ...
                Guid riid,
                out IntPtr ppv);
            // Recupera os atributos de um ou mais objetos de arquivo ou subpastas.
            // Valor de retorno: código de erro, se houver
            [PreserveSig]
           ...
           // Recupera o nome de exibição do objeto de arquivo ou subpasta especificada.
            ...
            // Define o nome de exibição de um objeto de arquivo ou subpasta, alterando o item
            // identificador no processo.
            // Valor de retorno: código de erro, se houver
            [PreserveSig]
            Int32 SetNameOf(
                IntPtr hwnd,
